...
export class AdminEditUserComponent implements OnInit {
  ...
  errorMessages = {
    firstName: [
      ...
      {type: 'maxlength', message: 'نام خانوادگی نمی تواند از 200 کاراکتر بیشتر باشد.'}
    ],
    lastName: [
    ...
    plaque: [
      {type: 'required', message: 'پلاک را وارد کنید.'}
    ],
  };
    ...
    this.service.getUserById(this.userId).subscribe((response) => {
      if (response.success === true) {
        ...
        if (this.user.city !== null) {
          this.stateOnChange(this.selectedState.code);
  ...
  submitForm(): void {
    ...
    this.service.editUser(this.userId, this.form.value).subscribe((response) => {
      if (response.success === true) {
