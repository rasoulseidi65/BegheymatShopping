...
@Component({
  selector: 'app-admin-edit-seller',
  templateUrl: './admin-edit-seller.component.html',
  styleUrls: ['./admin-edit-seller.component.css'],
...
export class AdminEditSellerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.service.getSellerById(this.sellerId).subscribe((response) => {

      this.getCategories();

      if (response.success === true) {
        this.seller = response.data;
        if (this.seller[0].hasOwnProperty('state')) {
          if (this.seller[0].state !== null) {
            this.selectedState = this.states.find(x => x.label === this.seller[0].state);

            if (this.seller[0].hasOwnProperty('city')) {
              if (this.seller[0].city !== null) {
                this.stateOnChange(this.selectedState.value);
                this.selectedCity = this.cities.find(x => x.label === this.seller[0].city);
              }
            }
          }
        }

        if (this.seller[0].hasOwnProperty('category')) {
          if (this.seller[0].category !== null) {
            this.selectedCategory = this.categories.find(x => x.title === this.seller[0].category);
          }
        }
  ...
  createBusinessForm(): void {
    this.businessForm = this.formBuilder.group({
      ...
      regNumCompany: new FormControl(
        this.seller[0].regNumCompany,
        [
          Validators.required
        ]
      ),
      economicCompany: new FormControl(
        this.seller[0].economicCompany,
        [
          Validators.required
        ]
      ),
      typeCompany: new FormControl(
        this.seller[0].typeCompany,
        [
          Validators.required
        ]
      ),
      imageSeller: new FormControl(
        this.seller[0].imageSeller,
        [
          Validators.required
        ]
      ),
      imageCertificate: new FormControl(
        this.seller[0].imageCertificate
      ),
      imageCompany: new FormControl(
        this.seller[0].imageCompany
      ),
      resume: new FormControl(
        this.seller[0].resume
      ),
    });
  }
  ...
  createPersonalForm(): void {
    this.personalForm = this.formBuilder.group({
      ...
      gender: new FormControl(
        this.seller[0].gender,
        [
          Validators.required
        ]
      ),
      idNumber: new FormControl(
        this.seller[0].idNumber,
        [
          Validators.required,
      ...
      birthDay: new FormControl(
        this.seller[0].birthDay,
        [
          Validators.required
        ]
      ),
      imageNationalcard: new FormControl(
        this.seller[0].imageNationalcard
      ),
    });
  }
  ...
  submitShopForm(): void {
    ...
    this.service.editSeller(this.sellerId, formData).subscribe((response) => {
      if (response.success === true) {
